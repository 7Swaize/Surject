...
public interface IServiceRegistry {
    ...
    IOpenGenericBindingBuilder AddOpenGeneric(Type openImplType, Lifetime lifetime);
    
    public IServiceRegistry Decorate<TContract, TDecorator>() where TDecorator : class, TContract;

    IBindingBuilder<TImpl> AddToCollection<TImpl>(Lifetime lifetime, int order = 0) where TImpl : class;
    IBindingBuilder<TImpl> AddPrimaryToCollection<TImpl, TContract>(int order = 0) where TImpl : class;
}